...
def add_watermark_to_image(image_path):
    try:
        ...
        try:
            # Replit thường dùng font DejaVu
...
def create_images_from_list(html_list, file_name):
    ...
    try:
        with sync_playwright() as p:
            # --- FIX CHO REPLIT: SỬ DỤNG CHROMIUM CỦA HỆ THỐNG ---
            # Replit.nix cài chromium vào đường dẫn hệ thống.
            # channel="chromium" giúp Playwright tìm bản cài đặt sẵn này.
            # executable_path có thể tự động được tìm thấy nhờ cấu hình env trong replit.nix
            ...
            print("     -> Đang khởi động trình duyệt (System Chromium)...")
            try:
                # Ưu tiên 1: Dùng channel chromium (bản hệ thống)
                browser = p.chromium.launch(channel="chromium", args=launch_args)
            except Exception as e1:
                try:
                    # Ưu tiên 2: Thử tìm đường dẫn cụ thể (thường thấy trên Nix)
                    ...
                    print(f"     ⚠️ Lỗi khởi động Chromium: {e2}")
                    raise e2
        ...
        print("💡 GỢI Ý: Kiểm tra file 'replit.nix' đã có 'pkgs.chromium', 'pkgs.mesa', 'pkgs.systemd' chưa.")
        if browser:
            try: browser.close()
            except: pass
        return False
...
# --- MAIN ---
def main():
    print(f"🚀 Bắt đầu... (Fixed: Replit Missing Dependencies)")
