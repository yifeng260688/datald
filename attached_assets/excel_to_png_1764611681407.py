...
WATERMARK_OPACITY = 90  # Độ đậm nhạt (0-255), càng thấp càng mờ. 30-50 là vừa đẹp.
GRID_COLS = 3 # Số lượng watermark theo chiều ngang
GRID_ROWS = 3 # Số lượng watermark theo chiều dọc
...
# --- CORE: TẠO HTML (Đã xóa phần Cover) ---
...
def add_watermark_to_image(image_path):
    ...
    try:
        ...
        # Cài đặt Font chữ
        # Cố gắng load font Arial, nếu không có thì dùng default (xấu hơn)
        font_size = int(width / 25) # Kích thước chữ dựa theo chiều rộng ảnh cho cân đối
        try:
            # Trên Windows thường có arial.ttf, Linux/Mac cần path khác nếu lỗi
        ...
        for r in range(GRID_ROWS):
            for c in range(GRID_COLS):
                ...
                # Tính kích thước chuỗi để căn giữa chính xác
                ...
                # Vẽ chữ lên layer trong suốt
                draw.text((x, y), WATERMARK_TEXT, font=font, fill=text_color)
        
        # Gộp layer chữ vào ảnh gốc
        combined = Image.alpha_composite(base_image, txt_layer)
        
        # Chuyển về RGB để lưu file png/jpg chuẩn
...
# --- MAIN ---
def main():
    print(f"🚀 Bắt đầu... (No Cover + Watermark 5x3)")
